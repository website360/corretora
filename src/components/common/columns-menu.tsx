...
export interface ColumnsMenuProps {
  /** Column ids in their current order. */
  order: string[];
  /** Hidden column ids. */
  hidden: string[];
  /** Map of column id → display label. */
  labels: Record<string, string>;
  /** Returns true if a column is pinned (cannot be moved or hidden). */
...
/** Generic "Colunas" popover with show/hide toggles and drag-to-reorder. */
export function ColumnsMenu({
  ...
  function onDrop(targetId: string) {
    ...
    if (isLocked(dragId) || isLocked(targetId)) return;
    const next = [...order];
